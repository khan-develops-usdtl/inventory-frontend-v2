...
@Component({
  selector: 'inventory-office-supply-request-items',
  templateUrl: './office-supply-request-items.component.html',
  styleUrls: ['./office-supply-request-items.component.scss']
})
export class OfficeSupplyRequestItemsComponent implements OnInit {
  ...
  constructor(
    ...
    private _officeSupplyItemsService: OfficeSupplyItemsService,
  ...
  getGeneralItems() {
    this.rowData = this._officeSupplyItemsService.getOfficeSupplyItems().pipe(map(iItemsRes => {
      ...
      next: res => {
        if (res === true) {
          this.selected.map(item => {
            this._officeSupplyItemsService.deleteOfficeSupplyItem(item.id).subscribe(() => this.getGeneralItems())
