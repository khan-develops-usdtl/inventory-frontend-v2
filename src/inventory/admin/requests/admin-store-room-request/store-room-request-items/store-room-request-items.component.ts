...
import { GeneralRequestViewService } from '../../admin-general-request/general-request-view/general-request-view.service';

@Component({
  selector: 'inventory-store-room-request-items',
  templateUrl: './store-room-request-items.component.html',
  styleUrls: ['./store-room-request-items.component.scss']
})
export class StoreRoomRequestItemsComponent implements OnInit {
  ...
  getGeneralItems() {
    this.rowData = this._storeRoomService.getStoreRoomItems().pipe(map(iItemsRes => {
      ...
      next: res => {
        if (res === true) {
          this.selected.map(item => {
            this._storeRoomService.deleteStoreRoomItem(item.id).subscribe(() => this.getGeneralItems())
