...
import { GeneralRequestViewService } from '../general-request-view/general-request-view.service';
import { GeneralRequestItemsService } from './general-request-items.service';

@Component({
  selector: 'inventory-general-request-items',
  templateUrl: './general-request-items.component.html',
  styleUrls: ['./general-request-items.component.scss']
})
export class GeneralRequestItemsComponent implements OnInit {
  ...
  constructor(
    ...
    private _generalRequestItemsService: GeneralRequestItemsService,
  ...
  getGeneralItems() {
    this.rowData = this._generalRequestItemsService.getGeneralItems().pipe(map(iItemsRes => {
      ...
      next: res => {
        if (res === true) {
          this.selected.map(item => {
            this._generalRequestItemsService.deleteGeneralItem(item.id).subscribe(() => this.getGeneralItems())
