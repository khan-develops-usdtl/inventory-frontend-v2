...
export class QualityService {
  ...
  getTotalItems(): Observable<IDepartment[]> {
    return this._http.get<IDepartment[]>(`${Constants.url}/quality/total`)
  }

  getItems(): Observable<IDepartment[]> {
    return this._http.get<IDepartment[]>(`${Constants.url}/quality`)
  }

  getTotalItem(item_id: number): Observable<IDepartment> {
    return this._http.get<IDepartment>(`${Constants.url}/quality/total/${item_id}`)
  }

  getItemsByMasterId(item_id: number): Observable<IDepartment[]> {
    return this._http.get<IDepartment[]>(`${Constants.url}/quality/master/${item_id}`)
  }

  deleteItem(id: number) {
    return this._http.delete(`${Constants.url}/quality/${id}`)
  }

  createItem(departmentItem: IDepartment): Observable<IDepartment> {
    return this._http.post<IDepartment>(`${Constants.url}/quality`, departmentItem)
  }
  ...
  updateItem(id: number, departmentItem: IDepartment, currentUser: IProfile,): Observable<IDepartment> {
    return this._http.patch<IDepartment>(
      `${Constants.url}/quality/${id}`, 
      { departmentItem: departmentItem, currentUser: currentUser, }
      )
  }

  sendEmailReport() {
    return this._http.get(`${Constants.url}/quality/email`)
  }
}
