...
import { MassSpecCategoryFilterComponent } from './mass-spec-category-filter/mass-spec-category-filter.component';
import { MassSpecCategoryRendererComponent } from './mass-spec-category-renderer/mass-spec-category-renderer.component';
import { MassSpecQuantityRendererComponent } from './mass-spec-quantity-renderer/mass-spec-quantity-renderer.component';
import { MassSpecService } from './mass-spec.service';



@Component({
  selector: 'inventory-mass-spec',
...
export class MassSpecComponent implements OnInit {
  ...
  constructor(
    ...
    this.frameworkComponents = {
      quantityRenderer: MassSpecQuantityRendererComponent,
      categoryRenderer: MassSpecCategoryRendererComponent,
      categoryFilter: MassSpecCategoryFilterComponent,
    }
    this.context = { massSpecComponent: this }
  }

  ngOnInit(): void {
    this._updaterService.currentMassSpecUpdater.subscribe({
      next: () => {
        this.getTotal()
      }
    })
  ...
  getTotal() {
    this.rowData = this._massSpecService.getTotalItems().pipe(map(items => {
      return items
    }))
  }
  ...
  send() {
    this._massSpecService.sendEmailReport().subscribe({
  ...
  handleDelete($event: any) {
    this.selected.map(item => {
      this._massSpecService.deleteItem(item.id).subscribe(() =>
        this._massSpecService.getItems().subscribe({
          next: () => {
            this.getTotal()
            this.selected = []
