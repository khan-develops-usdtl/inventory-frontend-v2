...
@Component({
  selector: 'inventory-store-room-request-step-three',
  templateUrl: './store-room-request-step-three.component.html',
  styleUrls: ['./store-room-request-step-three.component.scss']
})
export class StoreRoomRequestStepThreeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._updateService.currentGeneralRequestUpdater.subscribe({
      next: () => this.getConfirmedStoreRoomRequestItems()
    })
    this.getColumnDefs()
    this.getDefaultColDef()
    this.getConfirmedStoreRoomRequestItems()
  }

  getConfirmedStoreRoomRequestItems() {
    this.rowData = this._storeRoomRequestService.getStoreRoomRequestItems().pipe(map(storeRoomRequestItemsRes => {
      return storeRoomRequestItemsRes.filter(storeRoomRequestItem => storeRoomRequestItem.is_confirmed === true).map(generalRequestItem => {
